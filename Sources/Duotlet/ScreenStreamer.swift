...
final class ScreenStreamer {
    ...
    private func begin(displayID: CGDirectDisplayID, on target: NSScreen) async {
        ...
        do {
            ...
            let fresh = SCStream(filter: activeFilter, configuration: configuration, delegate: receiver)
            try fresh.addStreamOutput(
                receiver,
                type: .screen,
                sampleHandlerQueue: DispatchQueue(label: "Duotlet.frames", qos: .userInteractive)
            )
