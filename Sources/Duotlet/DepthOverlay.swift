...
final class DepthOverlay {
    ...
    private let buildQueue = DispatchQueue(label: "Duotlet.pictureUpload", qos: .userInteractive)
    ...
    private var revealEnvelope = PresentationReveal()
    private var presentationOpacity = PresentationOpacity()
    ...
    /// draw.
    private func reveal() {
        guard window != nil, !hasRevealed, renderer?.isReady == true else { return }
        hasRevealed = true
        revealEnvelope.reveal(at: CACurrentMediaTime())
    }
    ...
    func update(progress: Double, currentAngle: Double, tuning: DepthTuning, geometryStrength: Double = 1,
        ...
        // draining its outer autorelease pool. Return drawable references at
        // this frame boundary, before requesting another of the three surfaces.
        autoreleasepool { renderer.render(
            corners: perspectiveMotion.advance(to: geometry.corners(
                startAngle: geometryStart ?? startAngle,
                currentAngle: geometryAngle ?? currentAngle,
                viewingDistanceRatio: tuning.viewingDistance,
                recession: tuning.recession,
                screenSize: screenSize,
                strength: geometryStrength
            ), screenSize: screenSize, dt: dt, release: release),
